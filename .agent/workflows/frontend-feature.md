...
4. **Hook**: Create a Custom Hook in `src/frontend/src/hooks` to wrap the store and API logic. **Important**: All business logic/data fetching must be here, not in components.
5. **Components**: Build "dumb" UI components in `src/frontend/src/components/features` using shadcn/ui and semantic Tailwind classes (e.g., `bg-background`).
